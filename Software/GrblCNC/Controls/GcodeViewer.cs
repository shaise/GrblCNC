...
namespace GrblCNC
{
    public partial class GcodeViewer : UserControl
    {
        ...
        float charWidth = 0;
        Bitmap drawBmp = null;
        Brush backBrush, foreBrush, lineBrush;
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            lineBrush = new SolidBrush(Color.FromArgb(128, ForeColor));
            InitDimensions();
        }
        ...
        void UpdateBitmap()
        {
            ...
            using (Graphics g = Graphics.FromImage(drawBmp))
            {
                ...
                for (int i = 0; i < numVisibleLines; i++)
                {
                    ...
                    if (showLineNumbers)
                    {
                        string stnum = (drawLineIx + 1).ToString() + ":";
                        g.DrawString(stnum, Font, lineBrush, 0, i * Font.Height);
                        SizeF s = g.MeasureString(stnum, Font);
                        xpos += s.Width;
                    }
                    if (drawLineIx == highlightLine)
                    {
                        b = backBrush;
                        g.FillRectangle(foreBrush, xpos, i * Font.Height, Width, Font.Height);
                    }
                    else
                        b = new SolidBrush(ForeColor);
                    g.DrawString(lines[drawLineIx], Font, b, xpos, i * Font.Height);
                }
            }
        }

    ...
    public class Texture
    {
        ...
        public static Texture From(string imgResourceName)
        {
            if (textureDict.ContainsKey(imgResourceName))
                return textureDict[imgResourceName];
            object obj = Resources.ResourceManager.GetObject(imgResourceName);
            if (obj is Bitmap)
            {
                Texture txt = new Texture((Bitmap)obj);
                textureDict[imgResourceName] = txt;
                return txt;
            }
            // always return a default texture
            return Texture.From("awesomeface");
        }
